...
// --- VISTA MATRÍCULA (FULL: DISEÑO POR TURNOS + GESTIÓN AVANZADA + NO SE ROMPE) ---
function MatriculaView({ user }) {
  ...
  const LOGO_URL = "/icon-192.png"; // Asegurate que esta ruta sea correcta para tu proyecto
  ...
  const techOptions = (usersList||[]).filter(u => u.role === 'Equipo Técnico'); // Corrección para que no rompa al editar
  ...
  return (
    <div className="animate-in fade-in pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived?'bg-gray-600':'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
         <div className="flex justify-between items-center gap-4 flex-wrap">
             ...
             <div className="flex gap-2">
                 ...
                 {isSuperAdmin && <button onClick={()=>setShowDataManagement(true)} className="p-2 border border-white/30 rounded-xl hover:bg-white/10" title="Gestión"><UploadCloud size={20}/></button>}
                 ...
                 <button onClick={exportFiltered} className="p-2 border border-white/30 rounded-xl hover:bg-white/10" title="Excel"><Download size={20}/></button>
      ...
      {/* MODAL VER ALUMNO (CON DISEÑO AMARILLO/AZUL) */}
      {viewingStudent && !showForm && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"><div className="bg-gradient-to-r from-blue-600 to-cyan-500 p-6 text-white"><div className="flex justify-between"><h2 className="text-2xl font-bold">{viewingStudent.lastName}, {viewingStudent.firstName}</h2><button onClick={()=>setViewingStudent(null)}><X/></button></div><div className="flex gap-2 mt-4"><button onClick={()=>setActiveModalTab('info')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='info'?'bg-white text-blue-600':'bg-black/20'}`}>Datos</button><button onClick={()=>setActiveModalTab('history')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='history'?'bg-white text-blue-600':'bg-black/20'}`}>Bitácora</button></div></div><div className="p-6 overflow-y-auto">{activeModalTab==='info' ? (
      <div className="space-y-4 text-sm">
        {/* DATOS BÁSICOS */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
            <div className="bg-gray-50 p-2 rounded-xl border"><p className="text-[10px] text-gray-400 font-bold uppercase">Nivel</p><p className="font-bold text-gray-800">{viewingStudent.level || '-'}</p></div>
            <div className="bg-purple-50 p-2 rounded-xl border border-purple-100"><p className="text-[10px] text-purple-400 font-bold uppercase">DX</p><p className="font-bold text-purple-800">{viewingStudent.dx || '-'}</p></div>
            <div className="bg-gray-50 p-2 rounded-xl border"><p className="text-[10px] text-gray-400 font-bold uppercase">Género</p><p className="font-bold text-gray-800">{viewingStudent.gender || '-'}</p></div>
            <div className="bg-gray-50 p-2 rounded-xl border"><p className="text-[10px] text-gray-400 font-bold uppercase">Jornada</p><p className="font-bold text-gray-800">{viewingStudent.journey || '-'}</p></div>
        </div>
        
        {/* SECCIÓN ESCOLARIDAD (AMARILLO Y AZUL) */}
        <div><h4 className="font-bold text-gray-800 mb-2 flex items-center gap-2"><GraduationCap size={16} className="text-blue-500"/> Escolaridad 2026</h4>
        <div className="grid grid-cols-2 gap-4">
            <div className="bg-yellow-50 p-3 rounded-xl border border-yellow-200 relative">
                <span className="absolute top-0 right-0 bg-yellow-200 text-yellow-800 text-[9px] px-2 py-0.5 rounded-bl-xl font-bold uppercase">Mañana</span>
                <div className="space-y-1 mt-2">
                    <div><span className="text-[10px] text-yellow-700 font-bold block">Grupo</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.groupMorning || '-'}</p></div>
                    <div><span className="text-[10px] text-yellow-700 font-bold block">Docente</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.teacherMorning || '-'}</p></div>
                    <div><span className="text-[10px] text-yellow-700 font-bold block">Auxiliar</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.auxMorning || '-'}</p></div>
                </div>
            </div>
            <div className="bg-indigo-50 p-3 rounded-xl border border-indigo-200 relative">
                <span className="absolute top-0 right-0 bg-indigo-200 text-indigo-800 text-[9px] px-2 py-0.5 rounded-bl-xl font-bold uppercase">Tarde</span>
                <div className="space-y-1 mt-2">
                    <div><span className="text-[10px] text-indigo-700 font-bold block">Grupo</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.groupAfternoon || '-'}</p></div>
                    <div><span className="text-[10px] text-indigo-700 font-bold block">Docente</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.teacherAfternoon || '-'}</p></div>
                    <div><span className="text-[10px] text-indigo-700 font-bold block">Auxiliar</span><p className="font-bold text-gray-800 text-xs">{viewingStudent.auxAfternoon || '-'}</p></div>
                </div>
            </div>
        </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                <h4 className="font-bold text-green-600 mb-2 text-xs uppercase flex items-center gap-1"><Activity size={12}/> Salud</h4>
                <p className="text-xs mb-1"><span className="text-gray-400 font-bold">OBRA SOCIAL:</span><br/>{viewingStudent.healthInsurance || 'NO DECLARA'}</p>
                <p className="text-xs"><span className="text-gray-400 font-bold">VENCIMIENTO CUD:</span><br/>{getSafeDate(viewingStudent.cudExpiration) || '-'}</p>
             </div>
             <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                <h4 className="font-bold text-orange-600 mb-2 text-xs uppercase flex items-center gap-1"><User size={12}/> Familia</h4>
                <p className="text-xs mb-1"><span className="text-gray-400 font-bold">MADRE:</span> {viewingStudent.motherName} ({viewingStudent.motherContact})</p>
                <p className="text-xs mb-1"><span className="text-gray-400 font-bold">PADRE:</span> {viewingStudent.fatherName} ({viewingStudent.fatherContact})</p>
                <p className="text-xs"><span className="text-gray-400 font-bold">DIRECCIÓN:</span> {viewingStudent.address}</p>
             </div>
        </div>

      </div>) : (<div className="space-y-3">{viewingStudent.incidents?.map((inc,i)=>(<div key={i} className="bg-gray-50 p-3 rounded-xl border border-gray-100"><div className="flex justify-between"><span className="text-[10px] font-bold text-gray-400">{new Date(inc.date).toLocaleDateString()}</span><button onClick={()=>deleteIncident(viewingStudent.id, inc)}><Trash2 size={12} className="text-red-300"/></button></div><p className="font-bold text-sm">{inc.type}</p><p className="text-xs text-gray-500">{inc.author}</p></div>))}</div>)}</div><div className="p-4 border-t flex justify-end gap-2 bg-gray-50"><button onClick={()=>imprimirListado([viewingStudent])} className="px-4 py-2 bg-white border border-gray-300 rounded-xl text-gray-700 font-bold text-xs uppercase hover:bg-gray-100 flex gap-2 items-center"><FileText size={16}/> Imprimir Ficha</button><button onClick={()=>openEdit(viewingStudent)} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-xs uppercase hover:bg-blue-700 flex gap-2 items-center"><Edit3 size={16}/> Editar</button></div></div></div>)}
