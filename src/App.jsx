...
// --- VISTA MATRÍCULA (VERSIÓN COMPLETA: FILTROS + GESTIÓN + FICHA DETALLADA) ---
function MatriculaView({ user }) {
  ...
  const filteredStudents = students.filter(s => {
    ...
    // Filtro inteligente de docentes (busca en ambos turnos)
    if (filters.teacher !== 'all') {
        const tMorning = (s.teacherMorning || '').toLowerCase();
        const tAfternoon = (s.teacherAfternoon || '').toLowerCase();
        const search = filters.teacher.toLowerCase();
        if (!tMorning.includes(search) && !tAfternoon.includes(search)) return false;
    }
  ...
  const handleDelete = async (id) => { if(confirm("¿Borrar?")) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', id)); setViewingStudent(null); setShowForm(false); } };
  ...
  const imprimirListado = (list) => { 
     ...
     list.forEach(s => { h += `<div class="card"><div class="head"><div><h1>${s.lastName}, ${s.firstName}</h1><p>DNI: ${s.dni} - Edad: ${calculateAge(s.birthDate)}</p></div><img src="${LOGO_URL}"/></div><div class="grid"><div><span class="label">Nacimiento</span><div class="val">${getSafeDate(s.birthDate)}</div></div><div><span class="label">Diagnóstico</span><div class="val">${s.dx}</div></div><div><span class="label">Obra Social</span><div class="val">${s.healthInsurance||'-'}</div></div><div><span class="label">CUD Vto</span><div class="val">${getSafeDate(s.cudExpiration)}</div></div></div><br><h3>Escolaridad 2026</h3><div class="grid"><div><span class="label">Nivel</span><div class="val">${s.level||'-'}</div></div><div><span class="label">Jornada</span><div class="val">${s.journey||'-'}</div></div><div><span class="label">Aula</span><div class="val">${s.classroom||'-'}</div></div></div><br><div class="grid"><div><span class="label">Turno Mañana</span><div class="val">${s.groupMorning||'-'} (Prof: ${s.teacherMorning||'-'})</div></div><div><span class="label">Turno Tarde</span><div class="val">${s.groupAfternoon||'-'} (Prof: ${s.teacherAfternoon||'-'})</div></div></div><br><h3>Familia</h3><p><b>Madre:</b> ${s.motherName} (${s.motherContact})</p><p><b>Padre:</b> ${s.fatherName} (${s.fatherContact})</p><p><b>Dirección:</b> ${s.address}</p></div>`; }); 
  ...
  return (
    <div className="animate-in fade-in pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived?'bg-gray-600':'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
         <div className="flex justify-between items-center gap-4 flex-wrap">
             ...
             <div className="flex gap-2">
                 ...
                 {isSuperAdmin && <button onClick={()=>setShowDataManagement(true)} className="p-2 border border-white/30 rounded-xl hover:bg-white/10" title="Gestión de Datos"><UploadCloud size={20}/></button>}
                 {isSuperAdmin && <button onClick={()=>setShowStats(true)} className="p-2 border border-white/30 rounded-xl hover:bg-white/10" title="Estadísticas"><PieChart size={20}/></button>}
                 ...
                 <button onClick={exportFiltered} className="p-2 border border-white/30 rounded-xl hover:bg-white/10" title="Exportar Excel"><Download size={20}/></button>
         ...
         {!showArchived && <div className="mt-4 space-y-2"><div className="bg-white/20 p-2 rounded-xl flex items-center"><Search className="ml-2 opacity-70"/><input value={filterText} onChange={e=>setFilterText(e.target.value)} placeholder="Buscar alumno..." className="bg-transparent border-none outline-none text-white w-full font-bold placeholder-white/60 ml-2"/>{filterText && <button onClick={()=>setFilterText('')}><X/></button>}</div><div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide"><select value={filters.os} onChange={e=>setFilters({...filters, os:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Obra Social: Todas</option>{uniqueOS.map(o=><option key={o} value={o}>{o}</option>)}</select><select value={filters.group} onChange={e=>setFilters({...filters, group:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Grupo: Todos</option>{uniqueGroups.map(g=><option key={g} value={g}>{g}</option>)}</select><select value={filters.turn} onChange={e=>setFilters({...filters, turn:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Turno: Todos</option><option value="Mañana">Mañana</option><option value="Tarde">Tarde</option></select><select value={filters.level} onChange={e => setFilters({...filters, level: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Nivel: Todos</option><option value="INICIAL">INICIAL</option><option value="1° Ciclo">1° Ciclo</option><option value="2° Ciclo">2° Ciclo</option><option value="CFI">CFI</option></select><select value={filters.teacher} onChange={e => setFilters({...filters, teacher: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Docente: Todos</option>{staffOptions.map(u => <option key={u.id} value={u.fullName}>{u.fullName}</option>)}</select><select value={filters.dx} onChange={e => setFilters({...filters, dx: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">DX: Todos</option><option value="DI">DI</option><option value="TES">TES</option><option value="Otro">Otro</option></select></div></div>}
      </div>
      ...
      {/* MODAL VER ALUMNO (CON PESTAÑAS Y FICHA COMPLETA) */}
      {viewingStudent && !showForm && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"><div className="bg-gradient-to-r from-blue-600 to-cyan-500 p-6 text-white"><div className="flex justify-between"><h2 className="text-2xl font-bold">{viewingStudent.lastName}, {viewingStudent.firstName}</h2><button onClick={()=>setViewingStudent(null)}><X/></button></div><div className="flex gap-2 mt-4"><button onClick={()=>setActiveModalTab('info')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='info'?'bg-white text-blue-600':'bg-black/20'}`}>Datos</button><button onClick={()=>setActiveModalTab('history')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='history'?'bg-white text-blue-600':'bg-black/20'}`}>Bitácora</button></div></div><div className="p-6 overflow-y-auto">{activeModalTab==='info' ? (<div className="space-y-4 text-sm">
        <div className="grid grid-cols-2 gap-4"><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">DNI</span><p className="font-bold">{viewingStudent.dni}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">Nacimiento</span><p className="font-bold">{getSafeDate(viewingStudent.birthDate)}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">Obra Social</span><p className="font-bold">{viewingStudent.healthInsurance||'-'}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">CUD Vto</span><p className="font-bold">{getSafeDate(viewingStudent.cudExpiration)}</p></div></div>
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100"><h4 className="font-bold text-blue-800 mb-2 uppercase text-xs">Trayectoria 2026</h4><div className="grid grid-cols-2 gap-2 text-xs mb-2"><div><span className="text-blue-400 font-bold">Nivel</span><p>{viewingStudent.level || '-'}</p></div><div><span className="text-blue-400 font-bold">Jornada</span><p>{viewingStudent.journey || '-'}</p></div></div><div className="border-t border-blue-200 pt-2 text-xs"><div className="flex justify-between mb-1"><span>TM: {viewingStudent.groupMorning || '-'}</span><span className="font-bold">{viewingStudent.teacherMorning}</span></div><div className="flex justify-between"><span>TT: {viewingStudent.groupAfternoon || '-'}</span><span className="font-bold">{viewingStudent.teacherAfternoon}</span></div></div></div>
        <div className="bg-orange-50 p-4 rounded-xl border border-orange-100"><h4 className="font-bold text-orange-800 mb-2 uppercase text-xs">Familia y Contacto</h4><p><b>Madre:</b> {viewingStudent.motherName} ({viewingStudent.motherContact})</p><p><b>Padre:</b> {viewingStudent.fatherName} ({viewingStudent.fatherContact})</p><p><b>Dirección:</b> {viewingStudent.address}</p></div></div>) : (<div className="space-y-3">{viewingStudent.incidents?.map((inc,i)=>(<div key={i} className="bg-gray-50 p-3 rounded-xl border border-gray-100"><div className="flex justify-between"><span className="text-[10px] font-bold text-gray-400">{new Date(inc.date).toLocaleDateString()}</span><button onClick={()=>deleteIncident(viewingStudent.id, inc)}><Trash2 size={12} className="text-red-300"/></button></div><p className="font-bold text-sm">{inc.type}</p><p className="text-xs text-gray-500">{inc.author}</p></div>))}</div>)}</div><div className="p-4 border-t flex justify-end gap-2 bg-gray-50"><button onClick={()=>imprimirListado([viewingStudent])} className="px-4 py-2 bg-white border border-gray-300 rounded-xl text-gray-700 font-bold text-xs uppercase hover:bg-gray-100 flex items-center gap-2"><FileText size={16}/> Imprimir Ficha</button><button onClick={()=>openEdit(viewingStudent)} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-xs uppercase hover:bg-blue-700 flex items-center gap-2"><Edit3 size={16}/> Editar</button></div></div></div>)}

      {/* MODAL GESTIÓN DE DATOS (NUBE) */}
      {showDataManagement && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[80]"><div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl"><div className="flex justify-between mb-4"><h3 className="font-bold text-xl text-gray-800">Gestión de Datos</h3><button onClick={()=>setShowDataManagement(false)}><X/></button></div><div className="grid grid-cols-2 gap-3 mb-6"><button onClick={findDuplicates} className="p-3 bg-yellow-50 text-yellow-700 rounded-xl font-bold text-xs hover:bg-yellow-100 border border-yellow-200">🔍 Buscar Duplicados</button><button onClick={checkUnassigned} className="p-3 bg-red-50 text-red-700 rounded-xl font-bold text-xs hover:bg-red-100 border border-red-200">⚠️ Ver Sin Grupo</button><button onClick={limpiarEspaciosMasivo} className="p-3 bg-indigo-50 text-indigo-700 rounded-xl font-bold text-xs hover:bg-indigo-100 border border-indigo-200 col-span-2">✨ Limpiar Espacios en Nombres</button></div><div className="bg-gray-50 p-4 rounded-xl border border-gray-200"><h4 className="font-bold text-xs uppercase text-gray-500 mb-2">Importar / Exportar</h4><div className="flex gap-2"><button onClick={descargarBackup} className="flex-1 py-2 bg-white border rounded-lg text-xs font-bold">Descargar JSON</button><button onClick={handleBulkImport} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold">Importar JSON</button></div><textarea value={importJson} onChange={e=>setImportJson(e.target.value)} placeholder="Pegar JSON aquí..." className="w-full mt-2 p-2 text-xs border rounded-lg h-20"/></div></div></div>)}

      {/* MODAL ESTADÍSTICAS */}
      {showStats && (<div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4"><div className="bg-white rounded-[40px] w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 border-t-8 border-violet-600"><div className="flex justify-between items-center mb-6"><div><h3 className="text-2xl font-black text-violet-900 uppercase italic">Estadísticas</h3><p className="text-xs text-gray-500">Filtrado en tiempo real</p></div><button onClick={() => setShowStats(false)} className="bg-gray-100 p-2 rounded-full hover:bg-gray-200"><X size={20}/></button></div><div className="bg-violet-50 p-6 rounded-3xl text-center mb-6 border border-violet-100 shadow-inner"><span className="text-5xl font-black text-violet-600 block mb-2">{filteredStudents.length}</span><span className="text-xs font-bold text-violet-400 uppercase tracking-[4px]">Estudiantes Encontrados</span></div><p className="text-center text-xs text-gray-400">Usa los filtros de arriba para ver subtotales.</p></div></div>)}
      
      {/* ... MODALES SECUNDARIOS ... */}
      {showDupes && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[90]"><div className="bg-white rounded-3xl p-6 w-full max-w-2xl h-[80vh] flex flex-col"><div className="flex justify-between mb-4"><h3 className="font-bold">Posibles Duplicados ({potentialDupes.length})</h3><button onClick={()=>setShowDupes(false)}><X/></button></div><div className="flex-1 overflow-y-auto space-y-2">{potentialDupes.map((d,i)=>(<div key={i} className="flex gap-2 items-center bg-gray-50 p-2 rounded-xl"><div className="flex-1"><p className="font-bold">{d.original.firstName} {d.original.lastName}</p><span className="text-xs text-blue-500">Original</span></div><div className="flex-1"><p className="font-bold">{d.duplicate.firstName} {d.duplicate.lastName}</p><span className="text-xs text-red-500">Posible Copia</span></div><button onClick={()=>mergeStudents(d.original, d.duplicate)} className="px-3 py-1 bg-green-500 text-white rounded-lg text-xs font-bold">Fusionar</button></div>))}</div></div></div>)}
      ...
      {/* FORMULARIO EDITAR (MANTENER IGUAL) */}
