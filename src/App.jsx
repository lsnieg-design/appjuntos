...
// --- VISTA DASHBOARD (BOTÓN CUMPLEAÑOS FLOTANTE) ---
...
// --- VISTA MATRÍCULA (FILTRO OBRA SOCIAL + IMPRESIÓN INDIVIDUAL) ---
function MatriculaView({ user }) {
  ...
  const imprimirFichasMasivas = () => {
    if (filteredStudents.length > 20 && !confirm(`¿Imprimir ${filteredStudents.length} fichas?`)) return;
    imprimirListado(filteredStudents);
  };

  // Otras funciones (limpieza, backup, duplicados) se mantienen, solo agrego el render:
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived ? 'bg-gray-600' : 'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={imprimirFichasMasivas} title="Filtra la lista antes de imprimir." className="bg-white text-blue-600 p-2 px-3 rounded-xl transition flex items-center gap-2 text-xs font-black uppercase shadow-md hover:bg-blue-50 border border-white/20"><FileText size={18}/> Imprimir Fichas</button>
      ...
      {/* ... (Otros modales como showStats, showDupes, etc se mantienen del original) */}
