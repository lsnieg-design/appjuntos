...
// --- VISTA MATRÍCULA (FULL: GESTIÓN + FILTROS + FICHA + ANTI-CRASH) ---
function MatriculaView({ user }) {
  ...
  const filteredStudents = students.filter(s => {
    ...
    // Filtro inteligente de docentes
  ...
  // --- UTILS (CON PROTECCIÓN DE FECHAS) ---
  const getSafeDate = (dateString) => { 
      if (!dateString) return ''; 
      try { return dateString.includes('T') ? dateString.split('T')[0] : dateString; } 
      catch (e) { return ''; } 
  };
  ...
  const handleSave = async (e) => { 
      ...
      // Construimos el objeto manual para asegurar tipos
      const data = { 
          ...
          sup1Morning: formData.get('sup1Morning'), sup2Morning: formData.get('sup2Morning'), 
          groupAfternoon: formData.get('groupAfternoon'), teacherAfternoon: formData.get('teacherAfternoon'), 
          auxAfternoon: formData.get('auxAfternoon'), sup1Afternoon: formData.get('sup1Afternoon'), 
          sup2Afternoon: formData.get('sup2Afternoon'), classroom: formData.get('classroom'), 
          address: formData.get('address'), motherName: formData.get('motherName'), motherContact: formData.get('motherContact'), 
          fatherName: formData.get('fatherName'), fatherContact: formData.get('fatherContact'), 
          photoUrl: photoPreview || editingStudent?.photoUrl || '', 
          isActive: formData.get('isActive') === 'true', updatedAt: serverTimestamp() 
      }; 
      try { 
          if (editingStudent) { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', editingStudent.id), data); } 
          else { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'students'), { ...data, isActive: true, createdAt: serverTimestamp(), incidents: [] }); } 
  ...
  const markAsInactive = async (student) => { if(!confirm(`¿Dar de baja a ${student.firstName}?`)) return; try { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', student.id), { isActive: false }); setUnassignedList(prev => prev.filter(s => s.id !== student.id)); } catch(e) { alert("Error."); } };
  
  // --- HERRAMIENTAS GESTIÓN (DUPLICADOS, ETC) ---
  ...
  const handleDeleteAll = handleResetCycle; // Alias para el botón peligroso
  ...
  return (
    <div className="animate-in fade-in pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived?'bg-gray-600':'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
         ...
         {!showArchived && <div className="mt-4 space-y-2"><div className="bg-white/20 p-2 rounded-xl flex items-center"><Search className="ml-2 opacity-70"/><input value={filterText} onChange={e=>setFilterText(e.target.value)} placeholder="Buscar..." className="bg-transparent border-none outline-none text-white w-full font-bold placeholder-white/60 ml-2"/>{filterText && <button onClick={()=>setFilterText('')}><X/></button>}</div><div className="flex gap-2 overflow-x-auto pb-2"><select value={filters.os} onChange={e=>setFilters({...filters, os:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Obra Social: Todas</option>{uniqueOS.map(o=><option key={o} value={o}>{o}</option>)}</select><select value={filters.group} onChange={e=>setFilters({...filters, group:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Grupo: Todos</option>{uniqueGroups.map(g=><option key={g} value={g}>{g}</option>)}</select><select value={filters.turn} onChange={e=>setFilters({...filters, turn:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Turno: Todos</option><option value="Mañana">Mañana</option><option value="Tarde">Tarde</option></select><select value={filters.level} onChange={e => setFilters({...filters, level: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Nivel: Todos</option><option value="INICIAL">INICIAL</option><option value="1° Ciclo">1° Ciclo</option><option value="2° Ciclo">2° Ciclo</option><option value="CFI">CFI</option></select><select value={filters.teacher} onChange={e => setFilters({...filters, teacher: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Docente: Todos</option>{staffOptions.map(u => <option key={u.id} value={u.fullName}>{u.fullName}</option>)}</select><select value={filters.dx} onChange={e => setFilters({...filters, dx: e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">DX: Todos</option><option value="DI">DI</option><option value="TES">TES</option><option value="Otro">Otro</option></select></div></div>}
      </div>
