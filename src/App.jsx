...
// --- VISTA MATRÍCULA (CON SELECTOR DE ESTADO ACTIVO/BAJA) ---
function MatriculaView({ user }) {
  ...
  const filteredStudents = students.filter(s => {
    ...
    // Lógica del switch: Si showArchived es true, muestra inactivos. Si es false, muestra activos.
  ...
  const handleSave = async (e) => { 
      ...
      // Conversión forzada del estado activo/inactivo
      ...
      try { 
          if (editingStudent) { 
              ...
              // Al crear, forzamos que si no se eligió nada, sea true
              const newStatus = d.isActive !== undefined ? d.isActive : true;
              await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'students'), { ...d, isActive: newStatus, createdAt: serverTimestamp(), incidents: [] }); 
          } 
          setShowForm(false); setEditingStudent(null); setPhotoPreview(null); 
      } catch (err) { alert("Error: " + err.message); } 
  };
  
  const handleDelete = async (id) => { if(confirm("⚠️ ¿ESTÁS SEGURO DE ELIMINAR DEFINITIVAMENTE ESTE LEGAJO?\n\nSi solo quieres ocultarlo, cambia el estado a 'INACTIVO'.")) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', id)); setShowForm(false); setEditingStudent(null); } };
    ...
    <div className="animate-in fade-in pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived?'bg-gray-600':'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
         <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
             <div>
                <h2 className="text-3xl font-bold flex gap-2 items-center"><GraduationCap/> {showArchived?'Archivo':'Legajos 2026'}</h2>
                <p className="opacity-80 text-sm mt-1">{filteredStudents.length} alumnos encontrados</p>
             </div>
      ...
      <div className="space-y-3">
                      ...
                      <div className="flex gap-4 items-center">
                          ...
                          <div>
                              <h4 className="font-bold text-gray-800 flex items-center gap-2">
                                  ...
                                  {s.dx && <span className="bg-purple-100 text-purple-700 text-[9px] px-1.5 py-0.5 rounded border border-purple-200">{s.dx}</span>}
                              </h4>
                              <div className="flex gap-2 mt-1">
                                  ...
                                  {(s.groupMorning || s.groupAfternoon) ? 
                                    <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-100 font-bold">{s.groupMorning || s.groupAfternoon}</span> 
          ...
          {filteredStudents.length === 0 && <p className="text-center text-gray-400 py-10">No se encontraron estudiantes.</p>}
      </div>
      ...
      {/* FORMULARIO EDITAR (CON SELECTOR DE ESTADO + BOTÓN ELIMINAR) */}
      ...
      {/* SELECTOR DE ESTADO (AQUÍ ESTÁ LA MAGIA PARA RECUPERAR ALUMNOS) */}
      ...
      <div className="flex gap-2 pt-4 border-t"><button type="button" onClick={()=>setShowForm(false)} className="flex-1 py-3 text-gray-500 font-bold uppercase text-xs">Cancelar</button><button type="submit" className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold uppercase text-xs shadow-lg">Guardar</button>
      {/* BOTÓN ELIMINAR */}
