...
// --- APP PRINCIPAL (SIN BARRA GRIS + MENU PC) ---
function MainApp({ user, onLogout, onProfileUpdate }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 font-sans text-slate-800">
      {/* ESTE ESTILO QUITA LA BARRA GRIS PERO DEJA EL SCROLL */}
      ...
      <header className="bg-violet-800 text-white shadow-lg px-4 py-3 flex justify-between items-center z-50 sticky top-0">
        <div className="flex items-center space-x-3"><img src={LOGO_URL} className="w-10 h-8 object-contain" /><div><h1 className="font-bold text-sm leading-tight">Juntos a la Par</h1><p className="text-[10px] text-orange-200 uppercase font-bold">{user.firstName}</p></div></div>
        <div className="flex items-center gap-3">
      ...
      {/* CLASE no-scrollbar APLICADA AQUÍ */}
...
// --- VISTA MATRÍCULA (FORMULARIO BLINDADO + IMPRESIÓN OK) ---
function MatriculaView({ user }) {
  ...
  const filteredStudents = students.filter(s => {
    ...
    const matchesText = (s.firstName||'').toLowerCase().includes(txt) || (s.lastName||'').toLowerCase().includes(txt) || (s.dni||'').toString().includes(txt);
    ...
    if (filters.teacher !== 'all' && s.teacherMorning !== filters.teacher && s.teacherAfternoon !== filters.teacher) return false;
  ...
  // Funciones Seguras
  const getSafeDate = (d) => { if(!d) return ''; if(typeof d === 'string') return d.split('T')[0]; return ''; };
  const calculateAge = (d) => { if (!d) return '-'; const t = new Date(); const b = new Date(d); let a = t.getFullYear() - b.getFullYear(); if (t.getMonth() < b.getMonth() || (t.getMonth()===b.getMonth() && t.getDate()<b.getDate())) a--; return a; };
  ...
  const handleSave = async (e) => { e.preventDefault(); const fd = new FormData(e.target); const d = Object.fromEntries(fd.entries()); d.isActive = d.isActive==='true'; d.photoUrl = photoPreview || editingStudent?.photoUrl || ''; try { if(editingStudent) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', editingStudent.id), d); else await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'students'), {...d, createdAt: serverTimestamp(), incidents: []}); setShowForm(false); } catch(err){ alert("Error: "+err.message); } };
  const handleDelete = async (id) => { if(confirm("¿Borrar?")) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'students', id)); setShowForm(false); } };
  ...
  const imprimirListado = (list) => {
     ...
     let h = `<html><head><title>Fichas</title><style>body{font-family:sans-serif;padding:20px}.card{border:1px solid #ccc;padding:20px;margin-bottom:20px;page-break-after:always}.head{display:flex;justify-content:space-between;border-bottom:2px solid #6d28d9;padding-bottom:10px;margin-bottom:15px}h1{color:#4c1d95;margin:0}img{height:50px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.label{font-size:10px;color:#666;font-weight:bold;text-transform:uppercase}.val{font-weight:bold}</style></head><body>`;
     list.forEach(s => { h += `<div class="card"><div class="head"><div><h1>${s.lastName}, ${s.firstName}</h1><p>DNI: ${s.dni} - Edad: ${calculateAge(s.birthDate)}</p></div><img src="${LOGO_URL}"/></div><div class="grid"><div><span class="label">Nacimiento</span><div class="val">${getSafeDate(s.birthDate)}</div></div><div><span class="label">Diagnóstico</span><div class="val">${s.dx}</div></div><div><span class="label">Obra Social</span><div class="val">${s.healthInsurance||'-'}</div></div><div><span class="label">CUD Vto</span><div class="val">${getSafeDate(s.cudExpiration)}</div></div></div><br><h3>Escolaridad</h3><div class="grid"><div><span class="label">Turno Mañana</span><div class="val">${s.groupMorning||'-'} (Prof: ${s.teacherMorning||'-'})</div></div><div><span class="label">Turno Tarde</span><div class="val">${s.groupAfternoon||'-'} (Prof: ${s.teacherAfternoon||'-'})</div></div></div><br><h3>Familia</h3><p><b>Madre:</b> ${s.motherName} (${s.motherContact})</p><p><b>Padre:</b> ${s.fatherName} (${s.fatherContact})</p><p><b>Dirección:</b> ${s.address}</p></div>`; });
  ...
  // Render (Simplificado para evitar errores de sintaxis en el copy-paste)
  return (
    <div className="animate-in fade-in pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived?'bg-gray-600':'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
         <div className="flex justify-between items-center gap-4 flex-wrap">
             <div><h2 className="text-3xl font-bold flex gap-2"><GraduationCap/> {showArchived?'Archivo':'Legajos 2026'}</h2><p className="opacity-80">{filteredStudents.length} alumnos</p></div>
             <div className="flex gap-2">
                 <button onClick={()=>setShowArchived(!showArchived)} className="px-4 py-2 border border-white/30 rounded-xl text-xs font-bold uppercase hover:bg-white/10">{showArchived?'Ver Activos':'Ver Bajas'}</button>
                 {isSuperAdmin && <button onClick={()=>setShowDataManagement(true)} className="p-2 border border-white/30 rounded-xl hover:bg-white/10"><UploadCloud size={20}/></button>}
                 <button onClick={imprimirFichasMasivas} className="px-3 py-2 bg-white text-blue-600 rounded-xl text-xs font-black uppercase shadow hover:bg-blue-50 flex gap-2 items-center"><FileText size={16}/> Imprimir Fichas</button>
                 {!showArchived && <button onClick={openNew} className="p-3 bg-white text-blue-600 rounded-xl shadow hover:bg-blue-50"><Plus size={20}/></button>}
             </div>
         </div>
         {!showArchived && <div className="mt-4 space-y-2"><div className="bg-white/20 p-2 rounded-xl flex items-center"><Search className="ml-2 opacity-70"/><input value={filterText} onChange={e=>setFilterText(e.target.value)} placeholder="Buscar..." className="bg-transparent border-none outline-none text-white w-full font-bold placeholder-white/60 ml-2"/>{filterText && <button onClick={()=>setFilterText('')}><X/></button>}</div><div className="flex gap-2 overflow-x-auto pb-2"><select value={filters.os} onChange={e=>setFilters({...filters, os:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Obra Social: Todas</option>{uniqueOS.map(o=><option key={o} value={o}>{o}</option>)}</select><select value={filters.group} onChange={e=>setFilters({...filters, group:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Grupo: Todos</option>{uniqueGroups.map(g=><option key={g} value={g}>{g}</option>)}</select><select value={filters.turn} onChange={e=>setFilters({...filters, turn:e.target.value})} className="bg-white text-gray-700 text-xs p-2 rounded-lg font-bold"><option value="all">Turno: Todos</option><option value="Mañana">Mañana</option><option value="Tarde">Tarde</option></select></div></div>}
      </div>
      <div className="space-y-3">{filteredStudents.map(s => { const alert = getAlertStatus(s.incidents); return ( <div key={s.id} onClick={()=>{setViewingStudent(s); setActiveModalTab('info');}} className={`bg-white p-4 rounded-2xl shadow-sm border flex justify-between items-center cursor-pointer ${!s.isActive?'border-red-400 opacity-60':alert.status==='danger'?'border-red-500 border-l-4':'border-gray-100'}`}><div className="flex gap-4 items-center"><div className="w-12 h-12 bg-gray-200 rounded-xl overflow-hidden">{s.photoUrl?<img src={s.photoUrl} className="w-full h-full object-cover"/>:<div className="w-full h-full flex items-center justify-center font-bold text-gray-400">{s.firstName[0]}</div>}</div><div><h4 className="font-bold text-gray-800">{s.lastName}, {s.firstName}</h4><p className="text-xs text-gray-500">{s.groupMorning || s.groupAfternoon || 'Sin grupo'}</p></div></div><Eye className="text-gray-300"/></div> ); })}</div>
      
      {/* MODAL VER ALUMNO */}
      {viewingStudent && !showForm && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"><div className="bg-gradient-to-r from-blue-600 to-cyan-500 p-6 text-white"><div className="flex justify-between"><h2 className="text-2xl font-bold">{viewingStudent.lastName}, {viewingStudent.firstName}</h2><button onClick={()=>setViewingStudent(null)}><X/></button></div><div className="flex gap-2 mt-4"><button onClick={()=>setActiveModalTab('info')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='info'?'bg-white text-blue-600':'bg-black/20'}`}>Datos</button><button onClick={()=>setActiveModalTab('history')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase ${activeModalTab==='history'?'bg-white text-blue-600':'bg-black/20'}`}>Bitácora</button></div></div><div className="p-6 overflow-y-auto">{activeModalTab==='info' ? (<div className="space-y-4 text-sm"><div className="grid grid-cols-2 gap-4"><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">DNI</span><p className="font-bold">{viewingStudent.dni}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">Nacimiento</span><p className="font-bold">{getSafeDate(viewingStudent.birthDate)}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">Obra Social</span><p className="font-bold">{viewingStudent.healthInsurance||'-'}</p></div><div className="bg-gray-50 p-3 rounded-lg"><span className="text-xs font-bold text-gray-400 uppercase">CUD Vto</span><p className="font-bold">{getSafeDate(viewingStudent.cudExpiration)}</p></div></div><div className="bg-orange-50 p-4 rounded-xl border border-orange-100"><h4 className="font-bold text-orange-800 mb-2">Contacto</h4><p><b>Madre:</b> {viewingStudent.motherName} ({viewingStudent.motherContact})</p><p><b>Padre:</b> {viewingStudent.fatherName} ({viewingStudent.fatherContact})</p></div></div>) : (<div className="space-y-3">{viewingStudent.incidents?.map((inc,i)=>(<div key={i} className="bg-gray-50 p-3 rounded-xl border border-gray-100"><div className="flex justify-between"><span className="text-[10px] font-bold text-gray-400">{new Date(inc.date).toLocaleDateString()}</span><button onClick={()=>deleteIncident(viewingStudent.id, inc)}><Trash2 size={12} className="text-red-300"/></button></div><p className="font-bold text-sm">{inc.type}</p><p className="text-xs text-gray-500">{inc.author}</p></div>))}</div>)}</div><div className="p-4 border-t flex justify-end gap-2 bg-gray-50"><button onClick={()=>imprimirListado([viewingStudent])} className="px-4 py-2 bg-white border border-gray-300 rounded-xl text-gray-700 font-bold text-xs uppercase hover:bg-gray-100">Imprimir Ficha</button><button onClick={()=>openEdit(viewingStudent)} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-xs uppercase hover:bg-blue-700">Editar</button></div></div></div>)}

      {/* MODAL EDITAR (SOLUCIÓN AL CRASH) */}
      {showForm && (<div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-3xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto"><h3 className="text-xl font-bold mb-4">{editingStudent?'Editar':'Nuevo'} Legajo</h3><form onSubmit={handleSave} className="space-y-4"><div className="grid grid-cols-2 gap-3"><input name="firstName" defaultValue={editingStudent?.firstName} placeholder="Nombre" required className="p-3 bg-gray-50 rounded-xl w-full border outline-none font-bold text-sm"/><input name="lastName" defaultValue={editingStudent?.lastName} placeholder="Apellido" required className="p-3 bg-gray-50 rounded-xl w-full border outline-none font-bold text-sm"/></div><div className="grid grid-cols-2 gap-3"><input name="dni" type="number" defaultValue={editingStudent?.dni} placeholder="DNI" className="p-3 bg-gray-50 rounded-xl w-full border outline-none font-bold text-sm"/><input name="birthDate" type="date" defaultValue={getSafeDate(editingStudent?.birthDate)} className="p-3 bg-gray-50 rounded-xl w-full border outline-none font-bold text-sm text-gray-500"/></div>
      <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 space-y-2"><p className="text-xs font-bold text-blue-800 uppercase">Institucional</p><div className="grid grid-cols-2 gap-2"><select name="level" defaultValue={editingStudent?.level} className="p-2 rounded-lg border text-xs font-bold"><option value="">Nivel...</option><option value="INICIAL">INICIAL</option><option value="1° Ciclo">1° Ciclo</option><option value="2° Ciclo">2° Ciclo</option><option value="CFI">CFI</option></select><select name="dx" defaultValue={editingStudent?.dx} className="p-2 rounded-lg border text-xs font-bold"><option value="">DX...</option><option value="DI">DI</option><option value="TES">TES</option><option value="Otro">Otro</option></select></div><div className="grid grid-cols-2 gap-2"><input name="groupMorning" defaultValue={editingStudent?.groupMorning} placeholder="Grupo TM" className="p-2 rounded-lg border text-xs"/><input name="groupAfternoon" defaultValue={editingStudent?.groupAfternoon} placeholder="Grupo TT" className="p-2 rounded-lg border text-xs"/></div><div className="grid grid-cols-2 gap-2"><select name="teacherMorning" defaultValue={editingStudent?.teacherMorning} className="p-2 rounded-lg border text-xs"><option value="">Docente TM...</option>{staffOptions.map(u=><option key={u.id} value={u.fullName}>{u.fullName}</option>)}</select><select name="teacherAfternoon" defaultValue={editingStudent?.teacherAfternoon} className="p-2 rounded-lg border text-xs"><option value="">Docente TT...</option>{staffOptions.map(u=><option key={u.id} value={u.fullName}>{u.fullName}</option>)}</select></div></div>
      <div className="p-3 bg-green-50 rounded-xl border border-green-100 space-y-2"><p className="text-xs font-bold text-green-800 uppercase">Salud</p><input name="healthInsurance" defaultValue={editingStudent?.healthInsurance} placeholder="Obra Social" className="w-full p-2 rounded-lg border text-xs"/><input name="cudExpiration" type="date" defaultValue={getSafeDate(editingStudent?.cudExpiration)} className="w-full p-2 rounded-lg border text-xs text-gray-500"/></div>
      ...
      {/* ... (Mantener modales de gestión si los usas) ... */}
      {showDataManagement && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[80]"><div className="bg-white rounded-2xl p-6 w-full max-w-md"><h3 className="font-bold mb-4">Gestión</h3><button onClick={limpiarEspaciosMasivo} className="w-full py-3 bg-indigo-100 text-indigo-700 font-bold rounded-xl mb-2">Limpiar Espacios</button><button onClick={()=>setShowDataManagement(false)} className="w-full py-3 text-gray-500">Cerrar</button></div></div>)}
    </div>
  );
}
