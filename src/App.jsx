...
// --- APP PRINCIPAL (CORREGIDA: SIN BARRA GRIS + MENÚ RESPONSIVE) ---
function MainApp({ user, onLogout, onProfileUpdate }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 font-sans text-slate-800">
      {/* ESTILO PARA OCULTAR LA BARRA GRIS */}
      <style>{`
      ...
      {/* AQUÍ SE APLICA LA CLASE no-scrollbar */}
      ...
      <nav className="fixed bottom-0 w-full bg-white border-t border-violet-100 h-16 z-30 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] pb-safe">
        {/* GRILLA INTELIGENTE: 5 columnas en Móvil, 7 en PC. */}
        <div className="grid grid-cols-5 md:grid-cols-7 h-full max-w-5xl mx-auto px-2 relative">
          ...
          {/* Botones Extra (SOLO PC - Desplegados) */}
          ...
          {/* Menú Hamburguesa (SOLO MÓVIL - Comprimido) */}
          <div className="relative block md:hidden">
            <NavButton active={['matricula', 'resources', 'proyecto'].includes(activeTab)} onClick={() => setShowMoreMenu(!showMoreMenu)} icon={<List size={20} />} label="Más" />
...
function MatriculaView({ user }) {
  ...
  const uniqueOS = [...new Set(students.map(s => s.healthInsurance ? s.healthInsurance.toUpperCase().trim() : 'SIN COBERTURA'))].sort();
  ...
  const filteredStudents = students.filter(s => {
    ...
    // Filtro Obra Social
    const currentOS = s.healthInsurance ? s.healthInsurance.toUpperCase().trim() : 'SIN COBERTURA';
    const osMatch = filters.os === 'all' || currentOS === filters.os;

    let turnMatch = true;
    if (filters.turn === 'Mañana') turnMatch = !!s.groupMorning;
    if (filters.turn === 'Tarde') turnMatch = !!s.groupAfternoon;

    return textMatch && levelMatch && groupMatch && teacherMatch && turnMatch && dxMatch && genderMatch && journeyMatch && osMatch;
  });
  ...
  const descargarBackup = () => { if(!confirm("¿Descargar copia de seguridad completa (JSON)?")) return; const dataStr = JSON.stringify(students, null, 2); const blob = new Blob([dataStr], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `RESPALDO_MATRICULA_${new Date().toISOString().slice(0,10)}.json`; document.body.appendChild(link); link.click(); document.body.removeChild(link); };
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      <div className={`p-6 rounded-3xl shadow-lg text-white mb-6 transition-colors ${showArchived ? 'bg-gray-600' : 'bg-gradient-to-r from-blue-600 to-cyan-500'}`}>
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={imprimirFichasMasivas} className="bg-white text-blue-600 p-2 px-3 rounded-xl transition flex items-center gap-2 text-xs font-black uppercase shadow-md hover:bg-blue-50 border border-white/20"><FileText size={18}/> Imprimir Fichas</button>
        ...
        {!showArchived && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                ...
                <select value={filters.os} onChange={e => setFilters({...filters, os: e.target.value})} className="bg-white text-gray-700 border-none rounded-lg text-xs px-2 py-2 outline-none font-bold cursor-pointer hover:bg-gray-50 shadow-sm"><option value="all">O. Social: Todas</option>{uniqueOS.map(os => <option key={os} value={os}>{os}</option>)}</select>
      ...
      {/* --- MODAL PARA VER ESTUDIANTE (CON BOTÓN DE IMPRIMIR Y EDITAR) --- */}
            ...
            <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]"> 
                <div className={`bg-gradient-to-r p-6 text-white relative shrink-0 ${!viewingStudent.isActive ? 'from-gray-500 to-gray-700' : 'from-blue-600 to-cyan-500'}`}> 
                    ...
                    <div className="flex gap-2 mt-6">
                        <button onClick={() => setActiveModalTab('info')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition ${activeModalTab === 'info' ? 'bg-white text-blue-600 shadow-md' : 'bg-black/20 text-white/70 hover:bg-black/30'}`}>Datos Personales</button>
                        <button onClick={() => setActiveModalTab('history')} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition ${activeModalTab === 'history' ? 'bg-white text-blue-600 shadow-md' : 'bg-black/20 text-white/70 hover:bg-black/30'}`}>Bitácora</button>
                    </div> 
                </div> 
    ...
    <div className="flex flex-col h-full bg-slate-100 animate-in fade-in duration-500">
      <div className="bg-white p-4 shadow-sm z-10 sticky top-0 flex flex-col gap-3">
          <div className="flex justify-between items-center">
              ...
              <button onClick={handlePrint} className="bg-violet-100 text-violet-700 p-2 rounded-xl shadow-sm hover:bg-violet-200 transition" title="Imprimir Listas de Clase"><FileText size={24}/></button>
          </div>
      ...
      <div className="flex-1 overflow-x-auto p-6"><div className="flex gap-6 h-full">{groups.length === 0 && (<div className="m-auto text-center opacity-50"><LayoutDashboard size={48} className="mx-auto mb-2 text-gray-300"/><p className="font-bold text-gray-400">No hay grupos asignados.</p></div>)} {groups.map((g) => (<div key={g.name} className="min-w-[280px] w-[300px] flex flex-col h-full bg-white rounded-[30px] border border-gray-200 shadow-sm relative overflow-hidden group-hover:shadow-md transition"><div className={`p-4 border-b-4 ${turn==='morning'?'border-orange-400 bg-orange-50':'border-indigo-400 bg-indigo-50'}`}><h3 className="font-black text-gray-800 text-lg leading-none uppercase italic">{g.name}</h3><div className="flex flex-col gap-1 mt-2">{g.classroom && (<span className="inline-flex items-center gap-1 text-[10px] font-bold text-gray-600 bg-white px-2 py-1 rounded w-fit shadow-sm"><StartIcon size={10}/> Aula {g.classroom}</span>)}<p className="text-[10px] text-gray-500 font-medium truncate">DOCENTE: <span className="font-bold uppercase text-violet-700">{g.teacher || 'Sin asignar'}</span></p>{g.aux && <p className="text-[10px] text-gray-500 font-medium truncate">AUX: <span className="font-bold uppercase">{g.aux}</span></p>}{(g.sup1 || g.sup2) && (<div className="mt-1 pt-1 border-t border-gray-200/50"><p className="text-[9px] text-violet-600 font-bold truncate">SUP: {g.sup1 || ''} {g.sup2 ? `& ${g.sup2}` : ''}</p></div>)}</div></div><div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50/50">{g.students.map(s => (<div key={s.id} className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3 relative hover:scale-[1.02] transition-transform duration-200"><div onClick={() => { setSelectedStudent(s); setActiveTab('info'); }} className="w-12 h-12 rounded-full bg-gray-200 border-2 border-white shadow-sm flex-shrink-0 overflow-hidden cursor-pointer relative">{s.photoUrl ? <img src={s.photoUrl} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold">{s.firstName[0]}</div>}{s.lastIncident && new Date(s.lastIncident).toDateString() === new Date().toDateString() && (<div className="absolute bottom-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-white animate-pulse"></div>)}</div><div className="flex-1 min-w-0" onClick={() => { setSelectedStudent(s); setActiveTab('info'); }}><h4 className="font-bold text-gray-700 text-sm truncate cursor-pointer">{s.firstName} {s.lastName}</h4><p className="text-[10px] text-gray-400">Ver Ficha</p></div><button onClick={() => setShowBitacoraModal(s)} className="w-8 h-8 rounded-full bg-violet-100 text-violet-600 flex items-center justify-center shadow-sm hover:bg-violet-600 hover:text-white transition-colors">⚡</button></div>))}</div></div>))}</div></div>
